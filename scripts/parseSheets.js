...
// Pass the year to parse in as an argument. This assumes the year is the last
...
function parseSheet(rows) {
  ...
  for (let ii = DATA_ROW_START; ii < rows.length; ii++) {
    ...
    for (let jj = SPLIT_COL_START; jj < row.length; jj += 2) {
      const index = (jj - SPLIT_COL_START) / 2;

      const aidStation = AID_STATIONS[year][index];

      // Parse the value to get a valid time.
      const time = row[jj].split('-').filter((t) => t && t !== ':').pop();
      if (time) {
        splits.push({
